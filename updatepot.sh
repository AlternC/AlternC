#!/bin/sh
#
# Met a jour les fichiers pot.
# 
cd bureau
for i in `find ../modules -maxdepth 1 -type f \! -name "*~" -printf "%f "`
  do  
  xgettext -f../modules/$i -d${i}_tmp -plocales/ -Lphp -k_ -k__ --from-code=iso-8859-1 --msgid-bugs-address="i18n@alternc.org"
  YEAR=`date +%Y`
