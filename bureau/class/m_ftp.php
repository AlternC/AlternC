...
class m_ftp {
  ...
  function get_list() {
    ...
    $r=array();
    $db->query("SELECT id, name, password, homedir FROM ftpusers WHERE uid='$cuid' ORDER BY homedir;");
    if ($db->num_rows()) {
  ...
  function get_ftp_details($id) {
    ...
    $r=array();
    $db->query("SELECT id, name, password, homedir FROM ftpusers WHERE uid='$cuid' AND id='$id';");
    if ($db->num_rows()) {
      $db->next_record();
      $tr=preg_match("/^\/var\/alternc\/html\/.\/[^\/]*\/(.*)$/", $db->f("homedir"),$match);                  /*"*/
  ...
  function put_ftp_details($id,$prefixe,$login,$pass,$dir) {
    ...
    $db->query("UPDATE ftpusers SET name='".$prefixe.$login."', password='$pass', homedir='/var/alternc/html/$l/$lo/$dir', uid='$cuid' WHERE id='$id';");
    return true;
  }
  ...
  function add_ftp($prefixe,$login,$pass,$dir) {
    ...
    if ($quota->cancreate("ftp")) {
      $db->query("INSERT INTO ftpusers (name,password,homedir,uid) VALUES ('".$prefixe.$login."','$pass','/var/alternc/html/$l/$lo/$dir','$cuid')");
      return true;
    } else {
      $err->raise("ftp",5);
      return false;
